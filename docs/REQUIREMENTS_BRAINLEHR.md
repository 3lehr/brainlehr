...
| BDW-R05 | `keep` | MUSS | DECIDED | Der lokale, modell- und MCP-neutrale 0.1-Kern bleibt Produktbasis; Quellen, Geltung, Konflikt und Retrievalwirkung sind Kern, eine stabile API ist noch nicht versprochen. | Core / Produkt | `BDW-R05-AC1`: Derselbe Kern besteht einen lokalen MCP-Lauf mit austauschbarem Modellpfad und ausgewiesener Quelle/Geltung. | FAIL: `pytest tests/test_kern_modellneutral.py` — 3 von 4 grün. Der Kern liefert ohne Einbettungsmodell und reproduzierbar, führt aber weder `norm_rang` noch `gilt_ab` noch `source` in der Antwort. Die zweite Hälfte des AC ist damit nicht erfüllt. | Auswahl „Beibehalten“; README; RQ-001/002 |
...
| BDW-P05 | `A` | Profil | DECIDED | Nächste Produktgrenze ist Zielbild A: ein governierter Local-first-Memory-Kern. | Gesamtprodukt / Product Owner | `BDW-P05-AC1`: Der Kern reproduziert in mindestens 95 % des festgelegten Prüfkorpus Aussage, Quelle, Status und Gültigkeit innerhalb eines Abrufs. | FAIL: `python3 messungen/zielbild_a_vollstaendigkeit.py` — **0 von 35 (0,0 %)** gegen Schwelle 95 %. Einzeln: Aussage 20,0 %, Quelle 0,0 %, Status 0,0 %, Geltung 0,0 %. Der Engpass ist NICHT die Trefferquote: die drei übrigen Bestandteile werden gar nicht ausgeliefert (siehe `BDW-R05`). Deren Behebung hebt Quelle, Status und Geltung in einem Schritt auf das Niveau der Trefferquote. | Auswahl „A · governierter Local-first-Memory-Kern“; RQ-014/015 |
