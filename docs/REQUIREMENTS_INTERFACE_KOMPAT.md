...
| INT-API-003 | `GET /api/domaene-dienst` | Brainlehr | Atelier | Port und Lebenszeichen einer Domäne als JSON; drei Lagen unterscheidbar (nicht importiert / ohne Dienst / mit Dienst). Kein Rückrechnen aus Anzeige-HTML. | TEST-INT-API-003 |
...
| INT-REG-001 | Domänenregistry | Brainlehr | Atelier | Mehrere Domänen sind adressierbar; `einzelunternehmer` ist kein fester Wert im Klienten. **Gebaut** (`GET /api/domaene-dienst`, `DienstRegistry` in BrainlehrCore, Zweig `brainlehr/atelier` `38adf700`). **Rest offen:** `port`/`basisURL` in `DienstAufsicht` hängen noch an 8799 (sechs Leser), ebenso `DomaenenSeite.domaene` und der Mitstart-Schlüssel. | TEST-INT-REG-001 |
...
| INT-UPD-002 | Importkennung und Rücknahme | Brainlehr | Betrieb | Jeder Import trägt eine Kennung, über die genau dieser Import zurückgenommen wird. **Gebaut**: `speichere()` liefert `importkennung`, `nimm_import_zurueck()` entfernt genau diese Zeilen; in Kraft gesetzte Zeilen bleiben stehen und werden gezählt. | TEST-INT-UPD-002 |
| INT-SNAP-001 | Snapshotgrenze (`cb24f119`) | Brainlehr | Abruf/Prüfkorpus | Ein Lauf liest einen festgehaltenen Stand, nicht bei jedem Aufruf die gegenwärtige DB. **Gebaut und verdrahtet**: `kern/schnappschuss.py` (Online-Backup-API), `kern/messlauf_abrufguete.py` pinnt den ganzen Lauf auf einen Stand (`beb14580`). Vorher las der Messlauf direkt gegen den wachsenden Bestand. Jede Zahl trägt jetzt die Kennung ihres Standes. | TEST-INT-SNAP-001 |
...
| INT-BST-001 | Darstellungsbaustein (`bestandteile`) | `openlehr_X` | Atelier | Anforderung wählt aus einem **geschlossenen** Katalog (`BestandteilRegistry.swift` / `kern/bestandteile.py`); eine Domäne kann ihn nicht erweitern und lädt nichts nach. Ohne Version — der Baustein gehört zum Atelier. (ADR-026) | TEST-INT-BST-001 |
| INT-BST-002 | Fachpaket (rechnet, zeichnet nicht) | Paketautor | Dienst der Domäne | Wird mit SemVer-Major eingebunden wie jede Bibliothek, im Vertrauensraum der Domäne. Liefert Daten, nie Darstellung. (ADR-026) | TEST-INT-BST-002 |
| INT-BST-003 | Trennlinie der beiden Sorten | Architektur | beide Repos | Was im Atelier läuft, ist nie versioniert-nachladbar; was in der Domäne läuft, ist nie im geschlossenen Katalog. Ein Paket, das beides beansprucht, wird abgewiesen. (ADR-026) | TEST-INT-BST-003 |
| INT-BST-004 | Fehlender Baustein ist ein Befund | Domäne | Atelier | Fordert eine Domäne einen Bestandteil an, den der Katalog nicht kennt, wird das gemeldet — nicht stillschweigend ignoriert und nicht als Erlaubnisfrage behandelt. | TEST-INT-BST-004 |
