...
| **O2** kein Absender-Schutz | **behoben gegen Browser, nicht gegen Programme** | `91f096a` — Herkunftsprüfung auf 8799; auf 4599 Beschränkung auf Loopback statt `hasPrefix("ws")`. Vorher gemessen, dass der Fund scharf ist: `bauen.sh` baut ohne `-c release`, `#if DEBUG` ist also aktiv. **Nachtrag 2026-08-15T12:20, gemessen für ADR-020:** `Handler._herkunft_ok()` prüft **nur den `Origin`-Header**. Ein Browser setzt ihn selbst und kann ihn nicht fälschen — ein Programm setzt ihn frei. Solange alles auf `127.0.0.1` bindet, trägt das; **fällt diese Bindung, ist die Prüfung nicht schwächer, sondern wirkungslos.** Das ist die Bedingung, die vor einem Dienst im Netz zu erfüllen ist |
