...
knowledge_nodes/lessons_learned selbst). Rueckstand: die beiden Spalten
actor/session fehlen in Bestands-DBs, die vor diesem Auftrag angelegt wurden,
auf BEIDEN Tabellen. Additiv und idempotent, gleiches Muster wie
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        conn = sqlite3.connect(str(db_path))
        missing = missing_columns(conn)
        assert missing == {"knowledge_nodes": {"actor", "session"}, "lessons_learned": {"actor", "session"}}, missing
        conn.close()

        res1 = migrate(db_path, apply=True)
        assert res1["fehlende_spalten"] == {"knowledge_nodes": ["actor", "session"],
                                             "lessons_learned": ["actor", "session"]}, res1
        ...
        row_n = conn.execute("SELECT actor, session FROM knowledge_nodes WHERE id='n1'").fetchone()
        assert row_n == (None, None), row_n  # Bestandszeile bleibt NULL, kein erfundener Rueckfuellwert
        row_l = conn.execute("SELECT actor, session FROM lessons_learned WHERE id='L-1'").fetchone()
        assert row_l == (None, None), row_l
        conn.close()
    ...
    print("SELFTEST OK: actor/session auf knowledge_nodes UND lessons_learned nachgetragen, "
